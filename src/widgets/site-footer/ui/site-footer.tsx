        ...
        <nav aria-label="DUWORKS 서비스 도메인" className="mt-4 flex flex-col items-start gap-2">
          {liveServices.map((service) => (
            <a
              ...
              className="group flex min-h-9 items-center gap-2 text-footer-domain font-semibold text-white-soft transition-colors duration-300 hover:text-teal"
            >
              <span>{service.host}</span>
              <ArrowUpRightIcon className="size-4 shrink-0 text-muted transition-transform duration-300 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 group-hover:text-teal" />
            </a>
          ))}
        </nav>
